    ...
    @if (auth('web')->check() && auth('web')->user()->canCreateChildren())
        ...
        <section class="mb-8 rounded-lg border border-gray-200 bg-white p-5">
            <div class="mb-1 flex items-center gap-3">
                ...
                <h2 class="text-lg font-semibold">Rôles de mes organisations enfant</h2>
            </div>
            <p class="mb-4 text-sm text-gray-500">
                Rôles utilisables par chacune de vos organisations {{ auth('web')->user()->level->childLevel()->pluralLabel() }} :
            ...
            @foreach ($roleTables as $table)
                    ...
                    <form method="POST" action="{{ $table['addRoute'] }}">
                        ...
                        <div class="mb-6">
                            ...
                            <input id="{{ $table['addDialog'] }}-name" type="text" name="name"
                                value="{{ $errors->{"{$table['bagPrefix']}-add"}->has('name') ? old('name') : '' }}" required
                                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                            @error('name', "{$table['bagPrefix']}-add")<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                ...
                @if ($errors->{"{$table['bagPrefix']}-add"}->any())
