...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.organizations.store') }}" class="max-w-sm space-y-4">
        ...
        <div>
            ...
            <select id="parent_id" name="parent_id" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                ...
                @foreach ($organizations as $parent)
                    <option value="{{ $parent->id }}" data-level="{{ $parent->level->value }}" @selected((int) old('parent_id') === $parent->id)>
        ...
        <div>
            <label for="responsable_email_confirmation" class="block text-sm font-medium">Confirmer le courriel</label>
        ...
        <div>
            ...
            <input id="responsable_cell_phone" type="tel" inputmode="numeric" maxlength="14" name="responsable_cell_phone"
                value="{{ old('responsable_cell_phone') }}" required
                class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
